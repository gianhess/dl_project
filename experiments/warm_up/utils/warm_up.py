...
from utils import get_cifar10_loaders, train_step, seed_everything, train_and_test
...
def warm_up_reg (seed : int,
    ...
    model = resnet18(num_classes = 10)
    criterion = Criterion(weight_factor = weight_factor)
    optimizer = Optimizer(model.parameters(), lr = lr)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    ...
    progress = trange(max_epoch_warm_up, position=0)
    results = []
    wandb.init(project="warm-up", entity = 'pl-nc' , name = f'regularized_loss_{weight_factor}')
    for epoch in progress:
        # training step on half dataset
        progress.set_description(f"Epoch {epoch+1} of {max_epoch_warm_up} (warm up)")
