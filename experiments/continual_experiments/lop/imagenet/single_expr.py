...
def repeat_expr(params: {}):
    ...
    #initialize NC metrics per epoch
    # nc1_per_epoch = torch.zeros((num_tasks, num_epochs), dtype=torch.float)
    # nc2_per_epoch = torch.zeros((num_tasks, num_epochs), dtype=torch.float)
    # nc3_per_epoch = torch.zeros((num_tasks, num_epochs), dtype=torch.float)
    # nc4_per_epoch = torch.zeros((num_tasks, num_epochs), dtype=torch.float)
    ...
    for task_idx in range(num_tasks):
        ...
        for epoch_idx in tqdm(range(num_epochs)):
            ...
            with torch.no_grad():
                ...
                # nc1_per_epoch[task_idx][epoch_idx] = NC1(model=net, inputs=batch_x, targets=batch_y, num_classes=2)
                # nc2_per_epoch[task_idx][epoch_idx] = NC2(model=net)
                # nc3_per_epoch[task_idx][epoch_idx] = NC3(model=net, inputs=batch_x, targets=batch_y, num_classes=2)
                # nc4_per_epoch[task_idx][epoch_idx] = NC4(model=net, inputs=batch_x, targets=batch_y, num_classes=2)
                ...
                # print('accuracy for task', task_idx, 'in epoch', epoch_idx, ': train, ',
                #       train_accuracies[task_idx][epoch_idx], ', test,', test_accuracies[task_idx][epoch_idx], 'NC1_per_epoch: ', nc1_per_epoch[task_idx][epoch_idx], 
                #             'NC2_per_epoch: ', nc2_per_epoch[task_idx][epoch_idx], 'NC3_per_epoch: ', nc3_per_epoch[task_idx][epoch_idx], 'NC4_per_epoch: ', nc4_per_epoch[task_idx][epoch_idx])

                # wandb.log({"train_acc": train_accuracies[task_idx][epoch_idx],"test_acc": test_accuracies[task_idx][epoch_idx], "NC1_per_epoch": nc1_per_epoch[task_idx][epoch_idx],
                #         "NC2_per_epoch": nc2_per_epoch[task_idx][epoch_idx],"NC3_per_epoch": nc3_per_epoch[task_idx][epoch_idx], "NC4_per_epoch": nc4_per_epoch[task_idx][epoch_idx]})

        with torch.no_grad():
            #calculate NC matrics at the end of each task
            nc1[task_idx] = NC1(model=net, inputs=x_train, targets=y_train, num_classes=2)
            nc2[task_idx] = NC2(model=net)
            nc3[task_idx] = NC3(model=net, inputs=x_train, targets=y_train, num_classes=2)
            nc4[task_idx] = NC4(model=net, inputs=x_train, targets=y_train, num_classes=2)

            if params["wandb"]:
                wandb.log({"train_acc": train_accuracies[task_idx][epoch_idx],"test_acc": test_accuracies[task_idx][epoch_idx], "nc1": nc1[task_idx],
        ...
        if task_idx % save_after_every_n_tasks == 0:
            save_data(data={
                ...
                # 'nc1_per_epoch': nc1_per_epoch,
                # 'nc2_per_epoch': nc2_per_epoch,
                # 'nc3_per_epoch': nc3_per_epoch,
                # 'nc4_per_epoch': nc4_per_epoch,
                'nc1': nc1,
                'nc2': nc2,
                'nc3': nc3,
                'nc4': nc4,
